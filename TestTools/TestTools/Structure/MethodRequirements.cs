...
namespace TestTools.Structure
{
    ...
    {
        public MethodRequirements()
            : this(null)
        { 
        }

        public MethodRequirements(string name)
            : this(name, null, new ParameterOptions[0])
        {
        }

        public MethodRequirements(string name, Type returnType, params Type[] parameterTypes) 
            : this(name, returnType, parameterTypes.Select(t => new ParameterOptions(t)).ToArray())
        {
        }

        public MethodRequirements(string name, Type returnType, params ParameterOptions[] parameters)
        {
            Name = name;
